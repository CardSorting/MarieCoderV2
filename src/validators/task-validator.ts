...
import { validateString, validateArray, validateOptional } from './common'
...
export function validateCreateTaskRequest(body: unknown): CreateTaskRequest {
  ...
  const files = validateOptional(
    request.files,
    (val) => validateArray(val, 'files', 50, (item) => {
  ...
  const provider = validateOptional(
    request.provider,
    (val) => {
